...
*/
    ...
    void save_key_vec(){
        ...
        for (auto it = loopIndexContainer.begin(); it != loopIndexContainer.end(); ++it)
        {
            ...
            p.x = copy_cloudKeyPoses6D->points[ind].x;
            p.y = copy_cloudKeyPoses6D->points[ind].y;
            p.z = copy_cloudKeyPoses6D->points[ind].z;




            //  모든 key frame을 저장
            //key_vec_.push_back(p);
            ...
            if (!key_vec_.empty()){
                ...
                float dist = calc_point_dist(key_vec_[size], p);
                if (dist < 3.0f){
                    //  skip save
                }
                else{
